!!! note    This page is automatically generated from the source code, and is a bit rough.  If you have    trouble, check the [source code for this type](https://github.com/TerriaJS/terriajs/blob/master/lib/Models/GltfCatalogItem.js) or post a message to the [forum](https://groups.google.com/forum/#!forum/terriajs).A CatalogItem representing a GL Transmission Format (glTF) model.
...
The object is of the form {title:string, content:string, key: string, confirmation: boolean, confirmText: string, width: number, height: number}.`initialTimeSource`The value of the animation timeline at start. Valid options in config file are:
...
'NONE', 'BOTH', 'CAST', and 'RECEIVE'.`shortReport`A short report to show on the now viewing tab.`showsInfo`A value indicating whether this data source should show an info icon.`startTime`The start time, as an ISO8601 string, to use when this catalog item is active
on the timeline. The time affects things like lighting and shadows. The`stopTime`The stop time, as an ISO8601 string, to use when this catalog item is active
on the timeline. The time affects things like lighting and shadows. The`upAxis`The model's up-axis. By default models are y-up according to the glTF spec,
